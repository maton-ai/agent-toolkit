import {MatonMcpClient, type McpTool} from './mcp-client';
 ...
 *
 * @template T - The type of tools returned by this toolkit (e.g., ChatCompletionTool[], MatonTool[])
 */
export class ToolkitCore<T = McpTool[]> {
  /**
   * The MCP client that handles connections to mcp.maton.ai and tool execution.
   */
  readonly mcpClient: MatonMcpClient;
  readonly configuration: Configuration;
  private _initializer = new AsyncInitializer();
  private _tools: T;

  constructor(config: ToolkitConfig, emptyTools: T) {
    this.mcpClient = new MatonMcpClient({
      apiKey: config.apiKey,
      context: {
        connection: config.configuration.context?.connection,
      },
    ...
    if (!this._initializer.isInitialized) {
      throw new Error(
        'MatonAgentToolkit not initialized. ' +
          'Use `await createMatonAgentToolkit()` factory (recommended) or call `await toolkit.initialize()` first.'
      );
    }
  }
    ...
    if (!this._initializer.isInitialized) {
      console.warn(
        '[MatonAgentToolkit] Accessing tools before initialization. ' +
          'Use createMatonAgentToolkit() factory (recommended) or call await toolkit.initialize() first. ' +
